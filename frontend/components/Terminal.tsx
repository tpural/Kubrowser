        ...
        xterm.onData((data) => {
          ...
          if (data === "\r" || data === "\n") {
            ...
            if (!command.startsWith("kubectl")) {
              console.log(
                "[Terminal] Non-kubectl command detected:",
                command,
                "- closing popout"
              );
            ...
            const kubectlGetPodsMatch = command.match(
              /^kubectl\s+get\s+(?:po|pod|pods)(?:\s+.*)?$/i
            );
            if (kubectlGetPodsMatch && onCommandDetectedRef.current) {
              ...
              if (/-A\b|--all-namespaces/i.test(command)) {
                console.log(
                  "[Terminal] Detected -A or --all-namespaces - closing popout"
                );
                if (onCommandCloseRef.current) {
                  onCommandCloseRef.current();
                }
              ...
              console.log(
                "[Terminal] Opening pod list for namespace:",
                namespace
              );
            ...
            const kubectlGetNodesMatch = command.match(
              /^kubectl\s+get\s+(?:no|node|nodes)(?:\s+.*)?$/i
            );
      ...
      if (shouldDisconnect && wsRef.current) {
        ...
        if (
          ...
          if (xtermRef.current) {
            ...
            xtermRef.current.writeln(
              "Press Reconnect to establish a new connection."
            );
          }
